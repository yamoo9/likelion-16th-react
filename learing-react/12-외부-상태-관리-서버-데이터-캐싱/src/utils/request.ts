 ...
 *   method: 'POST',
 *   body: JSON.stringify({ name: 'Monica' }),
 * })
 *
 * @example
 * // 3. 인증 토큰이 필요한 요청 (헤더 확장)
 * const profile = await request<User>('/api/me', {
 *   headers: {
 ...
 *
 * @example
 * // 4. FormData 전송 (이미지/파일 업로드)
 * // Content-Type을 직접 설정하지 않아도 내부 로직이 자동으로 처리합니다.
...
export async function request<T>(
  ...
  // FormData 전송 시에는 Content-Type을 브라우저에 맡겨야 함 (Boundary 자동 생성)
