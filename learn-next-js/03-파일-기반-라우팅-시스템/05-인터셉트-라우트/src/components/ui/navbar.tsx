...
import { Book, LucideMonitorCog, ShoppingBag } from 'lucide-react'
        ...
        <div className="flex items-center gap-4 text-sm font-bold">
          ...
          <Link
            ...
            className={cn(
              'flex items-center gap-1.5 rounded-full border px-4 py-2 transition-all duration-200',
              isDashboardActive
                ? 'border-emerald-100 bg-cyan-50 text-cyan-600 shadow-sm'
                : 'border-transparent text-slate-500 hover:bg-cyan-50/50 hover:text-cyan-600',
            )}
          >
            <LucideMonitorCog
              className={cn(
                'h-4 w-4',
                isDashboardActive ? 'text-cyan-600' : 'text-slate-400',
              )}
            />
            <span>dashboard</span>
          </Link>
